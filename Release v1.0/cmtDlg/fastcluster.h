...
#include <stdint.h>		// std::int_fast32_t
#include "common.h"
...
parent elements until a root is found. An element it is a root if
nodes[it] is zero. To make subsequent searches faster, the entry for
...
public:
	...
	t_index Find(t_index idx) const {
		if (parent[idx] != 0) { // a ¡æ b
			t_index p = idx;
			idx = parent[idx];
			if (parent[idx] != 0) { // a ¡æ b ¡æ c
				do {
					idx = parent[idx];
				} while (parent[idx] != 0);
				do {
