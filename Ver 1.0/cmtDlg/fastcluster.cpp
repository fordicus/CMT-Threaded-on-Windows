...
class doubly_linked_list {
	...
	Typical use: for (it=L.start; L<size; it=L.succ[I])
	or
	for (it=somevalue; L<size; it=L.succ[I])
	*/
public:
	t_index start;
...
#pragma GCC diagnostic pop
	...
	for (t_index j = 1; j<N - 1; ++j) {
		...
		min = d[idx2];
		for (it = idx2; it<prev_node; it = active_nodes.succ[it]) {
			t_float tmp = D_(it, prev_node);
#if HAVE_DIAGNOSTIC
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal"
#endif
...
#pragma GCC diagnostic pop
		...
		for (; it<N; it = active_nodes.succ[it]) {
			t_float tmp = D_(prev_node, it);
#if HAVE_DIAGNOSTIC
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal"
#endif
...
#pragma GCC diagnostic pop
	...
	for (t_index j = 0; j<N - 1; ++j) {
		if (NN_chain_tip <= 3) {
			...
			min = D_(idx1, idx2);
			for (it = active_nodes.succ[idx2]; it<N; it = active_nodes.succ[it]) {
				if (D_(idx1, it) < min) {
					min = D_(idx1, it);
					idx2 = it;
				}
		...
		do {
			...
			for (it = active_nodes.start; it<idx2; it = active_nodes.succ[it]) {
				if (D_(it, idx2) < min) {
					min = D_(it, idx2);
					idx1 = it;
				}
		...
		case METHOD_METR_SINGLE:
			...
			*/
			...
			for (it = active_nodes.start; it<idx1; it = active_nodes.succ[it])
				f_single(&D_(it, idx2), D_(it, idx1));
			// Update the distance matrix in the range (idx1, idx2).
			for (; it<idx2; it = active_nodes.succ[it])
				f_single(&D_(it, idx2), D_(idx1, it));
			// Update the distance matrix in the range (idx2, N).
			for (it = active_nodes.succ[idx2]; it<N; it = active_nodes.succ[it])
				f_single(&D_(idx2, it), D_(idx1, it));
			break;
		...
		case METHOD_METR_COMPLETE:
			...
			*/
			// Update the distance matrix in the range [start, idx1).
			for (it = active_nodes.start; it<idx1; it = active_nodes.succ[it])
				f_complete(&D_(it, idx2), D_(it, idx1));
			// Update the distance matrix in the range (idx1, idx2).
			for (; it<idx2; it = active_nodes.succ[it])
				f_complete(&D_(it, idx2), D_(idx1, it));
			// Update the distance matrix in the range (idx2, N).
			for (it = active_nodes.succ[idx2]; it<N; it = active_nodes.succ[it])
				f_complete(&D_(idx2, it), D_(idx1, it));
			break;
		...
		case METHOD_METR_AVERAGE: {
			...
			*/
			...
			for (it = active_nodes.start; it<idx1; it = active_nodes.succ[it])
				f_average(&D_(it, idx2), D_(it, idx1), s, t);
			// Update the distance matrix in the range (idx1, idx2).
			for (; it<idx2; it = active_nodes.succ[it])
				f_average(&D_(it, idx2), D_(idx1, it), s, t);
			// Update the distance matrix in the range (idx2, N).
			for (it = active_nodes.succ[idx2]; it<N; it = active_nodes.succ[it])
				f_average(&D_(idx2, it), D_(idx1, it), s, t);
			break;
		}
		...
		case METHOD_METR_WEIGHTED:
			...
			*/
			// Update the distance matrix in the range [start, idx1).
			for (it = active_nodes.start; it<idx1; it = active_nodes.succ[it])
				f_weighted(&D_(it, idx2), D_(it, idx1));
			// Update the distance matrix in the range (idx1, idx2).
			for (; it<idx2; it = active_nodes.succ[it])
				f_weighted(&D_(it, idx2), D_(idx1, it));
			// Update the distance matrix in the range (idx2, N).
			for (it = active_nodes.succ[idx2]; it<N; it = active_nodes.succ[it])
				f_weighted(&D_(idx2, it), D_(idx1, it));
			break;
		...
		case METHOD_METR_WARD:
			...
			*/
			// Update the distance matrix in the range [start, idx1).
			//t_float v = static_cast<t_float>(members[it]);
			for (it = active_nodes.start; it<idx1; it = active_nodes.succ[it])
				f_ward(&D_(it, idx2), D_(it, idx1), min,
				size1, size2, static_cast<t_float>(members[it]));
			// Update the distance matrix in the range (idx1, idx2).
			for (; it<idx2; it = active_nodes.succ[it])
				f_ward(&D_(it, idx2), D_(idx1, it), min,
				size1, size2, static_cast<t_float>(members[it]));
			// Update the distance matrix in the range (idx2, N).
			for (it = active_nodes.succ[idx2]; it<N; it = active_nodes.succ[it])
				f_ward(&D_(idx2, it), D_(idx1, it), min,
				size1, size2, static_cast<t_float>(members[it]));
			break;
...
class binary_min_heap {
	...
	contains the minimum. The lists I and R are made such that H[it] = A[I[it]]
	and R[I[it]] = it.
...
public:
	...
	void heapify() {
		// Arrange the indices I and R so that H[it] := A[I[it]] satisfies the heap
		// condition H[it] < H[2*it+1] and H[it] < H[2*it+2] for each it.
		//
		// Complexity: Θ(size)
	...
	void update(const t_index idx, const t_float val) const {
		// Update the element A[it] with val and re-arrange the indices to preserve
		// the heap condition.
		if (val <= A[idx])
			update_leq(idx, val);
		else
...
private:
	void update_leq_(t_index it) const {
		t_index j;
		for (; (it>0) && (H(it)<H(j = (it - 1) >> 1)); it = j)
			heap_swap(it, j);
	}

	void update_geq_(t_index it) const {
		t_index j;
		for (; (j = 2 * it + 1)<size; it = j) {
			if (H(j) >= H(it)) {
				++j;
				if (j >= size || H(j) >= H(it)) break;
			}
	...
	void heap_swap(const t_index it, const t_index j) const {
		// Swap two indices.
		t_index tmp = I[it];
		I[it] = I[j];
		I[j] = tmp;
	...
	inline t_float H(const t_index it) const {
		return A[I[it]];
	}
...
static void generic_linkage(const t_index N, t_float * const D, t_members * const members, cluster_result & Z2) {
	...
	*/
	...
	auto_array_ptr<t_index> row_repr(N); // row_repr[it]: node number that the
	// it-th row represents
	doubly_linked_list active_nodes(N);
	...
	// n_nghbr[it] = argmin_{j>it} D(it,j) for it in range(N-1)
	t_float const * DD = D;
	for (it = 0; it<N_1; ++it) {
		min = std::numeric_limits<t_float>::infinity();
		for (idx = j = it + 1; j<N; ++j, ++DD) {
#if HAVE_DIAGNOSTIC
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal"
#endif
...
#pragma GCC diagnostic pop
	...
	for (it = 0; it<N_1; ++it) {
		...
		mindist[it] stores a lower bound on the minimum distance of the point it to
		all points of higher index:

		mindist[it] ≥ min_{j>it} D(it,j)

		Normally, we have equality. However, this minimum may become invalid due
		to the updates in the distance matrix. The rules are:

		1) If mindist[it] is equal to D(it, n_nghbr[it]), this is the correct
		minimum and n_nghbr[it] is a nearest neighbor.

		2) If mindist[it] is smaller than D(it, n_nghbr[it]), this might not be the
		correct minimum. The minimum needs to be recomputed.

		3) mindist[it] is never bigger than the true minimum. Hence, we never
		...
		fulfilled. This makes sure that the minima in the rows D(it,it+1:)of D are
		...
		*/
		...
		active_nodes.remove(idx1);
		// Index idx2 now represents the new (merged) node with label N+it.
		row_repr[idx2] = N + it;
...
#pragma GCC diagnostic pop
	...
	for (t_index j = 1; j<N - 1; ++j) {
		...
		for (it = idx2; it<N; it = active_nodes.succ[it]) {
			t_float tmp = dist(it, prev_node);
#if HAVE_DIAGNOSTIC
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal"
#endif
...
static void generic_linkage_vector(const t_index N,
	...
	*/
	...
	for (it = 0; it<N_1; ++it) {
		min = std::numeric_limits<t_float>::infinity();
		t_index idx;
		for (idx = j = it + 1; j<N; ++j) {
			t_float tmp;
			switch (method) {
			case METHOD_VECTOR_WARD:
				tmp = dist.ward_initial(it, j);
				break;
			default:
				tmp = dist.template sqeuclidean<true>(it, j);
			}
			if (tmp<min) {
				min = tmp;
				idx = j;
		...
		switch (method) {
		case METHOD_VECTOR_WARD:
			mindist[it] = t_dissimilarity::ward_initial_conversion(min);
			break;
		default:
			mindist[it] = min;
		}
...
static void generic_linkage_vector_alternative(const t_index N,
	...
	*/
	...
	for (it = N; it<N + N_1; ++it) {
		...
		maintains a flag whether a node is inactive. If n_nghbr[it] points to an
		active node, the entries nn_distances[it] and n_nghbr[it] are valid,
		...
		*/
		...
		if (it<2 * N_1) {
			...
			case METHOD_VECTOR_WARD:
			case METHOD_VECTOR_CENTROID:
				dist.merge(idx1, idx2, it);
				break;

			case METHOD_VECTOR_MEDIAN:
				dist.merge_weighted(idx1, idx2, it);
				break;
			...
			if (method == METHOD_VECTOR_WARD) {
				...
				but maybe bigger than max(d1,d2).
				*/
				min = dist.ward_extended(active_nodes.start, it);
				for (j = active_nodes.succ[active_nodes.start]; j<it;
					j = active_nodes.succ[j]) {
					t_float tmp = dist.ward_extended(j, it);
					if (tmp < min) {
						min = tmp;
						n_nghbr[it] = j;
					}
				...
				but maybe smaller than min(d1,d2).
				*/
				min = dist.sqeuclidean_extended(active_nodes.start, it);
				for (j = active_nodes.succ[active_nodes.start]; j<it;
					j = active_nodes.succ[j]) {
					t_float tmp = dist.sqeuclidean_extended(j, it);
					if (tmp < min) {
						min = tmp;
						n_nghbr[it] = j;
					}
			...
			nn_distances.replace(idx1, it, min);
		}
	}
}
