...
void CcmtDlgDlg::OnBnClickedOk()
{
	...
	while (g.status == eRUN)
	{
		...
		//---------------------------------------------------------------------------
		g_isRedraw = g.QueryThreadChange(threadStat, threadStatOld);
		if (g_isRedraw) this->OnPaint();
		g.ReserveThreadStat(threadStat, threadStatOld);
		...
		if (g_selDetector == detector_GFTT) g_detector = Config_GFTT();
		else if (g_selDetector == detector_FAST) g_detector = Config_FAST();
		...
		//---------------------------------------------------------------------------
		vector<UINT> VALID = g.QueryActiveThread(threadStat);
		...
		{	// Capture
			...
			{
				//----------------------------------------------------------------------------
				// "C:\\Users\\VLSI-CUDA\\Desktop\\cmtResult\\LocalTimeCode().avi"
				//----------------------------------------------------------------------------
				vidPath += LocalTimeCode(); vidPath += ".avi";
	...
	while (g.status == eRUN)
	{
		...
		{	// Capture
			...
			else if (g_captureStat == capture_IDLE)
			{
				...
				g_writer.open(vidPath, CV_FOURCC('M', 'P', 'E', 'G'), 30, Size(g.cols, g.rows), TRUE);
				g_captureStat = capture_RUNNING;
			}
		}
