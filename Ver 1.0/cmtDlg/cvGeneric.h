// Robinus Fuser. 2015-09-15 22:45
...
#define consoleSweep		"\r\n \r\n \r\n \r\n "
#define notify_ONE			"Allow only one Tracker Process."
#define notify_FIX			"Turn of the Session first."
#define notify_EOF			"End of Track."
...
// Global Functions
//============================================================================
Ptr<FeatureDetector> inline Config_GFTT(VOID)
{
	UINT	maxCorners = 700;
...
Ptr<FeatureDetector> inline Config_FAST(VOID)
{
	UINT threshold = 20;
...
VOID inline SecureEndThread(INT threadStat[])
{
	for (register UINT id = 0; id < maxThreads; ++id)
	{
		...
		while (RETRY)
		{
			warpEvents[id].SetEvent();
			if (threadStat[id] == thread_VOID) RETRY = FALSE;
		}
	}
}
...
string inline LocalTimeCode(VOID)
{
	SYSTEMTIME systime;
	GetLocalTime(&systime);
...
public :
	...
	VOID Create(_T idx, string& name){
		// this->gate = TRUE;
		this->job = tCREATE;
		this->idx = idx;		
		this->name = name;		
	...
	template<typename _T>
	VOID Destruct(_T idx, string& name){
		// this->gate = TRUE;
		this->job = tDESTRUCT;
		this->idx = idx;
		this->name = name;		
	...
	template<typename _T>
	VOID Control(_T idx, string& name){
		// this->gate = TRUE;
		this->job = tCONTROL;
		this->idx = idx;		
		this->name = name;
...
public :
	...
	// Variables
	//========================================================================
	LARGE_INTEGER		lnFreqStart, lnFreqStop, lnStart, lnStop;
	ULONGLONG			procTime;
	//------------------------------------------------------------------------
	vector<string>		wnd;
	vector<Mat>			img;
	...
	VOID ResetQueryPerformance(VOID);
	ULONGLONG QueryPerformance(VOID);
	...
	vector<UINT> QueryActiveThread(INT* threadStat);
	VOID ReserveThreadStat(INT threadStat[], INT threadStatOld[]);
	BOOL QueryThreadChange(INT threadStat[], INT threadStatOld[]);
...
//----------------------------------------------------------------------------
// Process-time Measurement in [ms]
// Should call ResetQueryPerformance() at every start of the Query-sequence
//============================================================================
VOID inline cvGENERIC::ResetQueryPerformance()
{
	::QueryPerformanceFrequency(&this->lnFreqStart);
	::QueryPerformanceCounter(&this->lnStart);
}

ULONGLONG inline cvGENERIC::QueryPerformance()
{
	::QueryPerformanceCounter(&this->lnStop);
	::QueryPerformanceFrequency(&this->lnFreqStop);	

	this->procTime = static_cast<ULONGLONG>(
			( (this->lnStop.QuadPart - this->lnStart.QuadPart) * 1000000 ) /
			( (this->lnFreqStart.QuadPart + this->lnFreqStop.QuadPart ) / 2 )
		);
...
{
	this->wnd.clear();
	this->wnd.push_back("wnd_00"); namedWindow(wnd[0]);
	return this->wnd[0];
}
...
string inline cvGENERIC::LoadVid(BOOL CreatDir)
{
	...
	if (vid.isOpened())
	{
		Mat frm;
		vid >> frm;
		this->rows = vid.get(CAP_PROP_FRAME_HEIGHT);
		this->cols = vid.get(CAP_PROP_FRAME_WIDTH);
	}
...
vector<UINT> inline cvGENERIC::QueryActiveThread(INT* threadStat)
{
	vector<UINT> ACTIVE;
...
VOID inline cvGENERIC::ReserveThreadStat(INT threadStat[], INT threadStatOld[])
{
	for (register UINT id = 0; id < maxThreads; ++id)
...
BOOL inline cvGENERIC::QueryThreadChange(INT threadStat[], INT threadStatOld[])
{
